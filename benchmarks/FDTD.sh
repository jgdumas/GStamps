...
if [ "$TDoS" -gt 0 ]; then
    ...
    echo "s $EREAC $ECOVE $REAC $COVE $MOSS" 1>&2
    ...
    for j in $(seq $JSTR $MAXKpS)
      ...
      Check $BASI $KBAS $JBAS $j $EREAC $ECOVE $REAC $COVE $MOSS 1>&2
      echo -n $j
      Run $BASI $KBAS $JBAS $EREAC $j
      Run $BASI $KBAS $JBAS $ECOVE $j
    ...
    echo -e -n "${GRE}k s correctness${NC} " 1>&2
    echo "k $EREAC $ECOVE $REAC $COVE $MOSS" 1>&2
    (printf '=%.0s' {1..50} ; printf '\n') 1>&2

    for j in $(seq $JSTR $KBAS)
      do
      Check $BASI $j $MAXKpS $MAXKpS $EREAC $ECOVE $REAC $COVE $MOSS 1>&2
      echo -n $j
      Run $BASI $j $MAXKpS $EREAC $MAXKpS
      Run $BASI $j $MAXKpS $ECOVE $MAXKpS
