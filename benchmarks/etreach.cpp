...
inline bint _ET_LReach(const Iterator& start, const Iterator& end,
...
inline bint ET_LReach(const List& points, const stype_t s, const int verbose) {
    StTimer chrono; chrono.start();
    const bint max( _ET_LReach(points.begin(), points.end(), s, verbose) );
    chrono.stop();

    if (verbose>0) std::clog << "#[ERange(" << size_t(s) << ")]: " << max
                             << ' ' << chrono <<std::endl;
...
int tmain(int argc, char **argv, stype_t s) {
    ...
    std::cout << ET_LReach(points, s, verbose) << std::endl;
